...
function markerVM() {
    ...
    self.drawPolygons = function() {
        ...
        svg.selectAll("circle")
            ...
            .append("circle")
            .attr("cx", function(d) {
                return scaleX(d.points[0].x);
            })
            .attr("cy", function(d) {
                return scaleY(d.points[0].y);
            })
        ...
        svg.selectAll("text")
            ...
            .attr("y", function(d) {
                return scaleY(d.points[0].y) + fontSize / 2.5;
            })
            .text(function(d) {
                return self.polygons().indexOf(d);
            })
    ...
    self.createPoint = function() {
        ...
        if (points.length == 1) {
            ...
            point = {
                "x": mouseScaleX(point[0]),
                "y": mouseScaleY(point[1])
            };
            points.push(point);
        } else {
    ...
    self.loadPolygons = function() {
        d3.json("/pic/" + pic_id + "/polygons",
            function(json) {
                ...
                self.updatePolygons();
                self.currDefect(self.polygons()[0]);
                self.canSave(self.currDefect() !== undefined && self.currDefect()
                    .points.length > 0);
            });
    };
